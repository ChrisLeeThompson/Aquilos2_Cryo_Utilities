...
def _new_session_id() -> str:
    ...
    :func:`aquilos2_cu.stage_positions.positions_model._new_id` and
    :func:`aquilos2_cu.cryo.activity_records._new_instance_id` —
...
class SessionLog(QObject):
    ...
    def formatParameters(self, activity_id: str, params: Any) -> str:
        ...
        Sputter Coat speaks mA and Pa, GIS Deposition speaks port names
        and seconds, Argon Purge speaks cycles. The delegate doesn't
        care; it just shows what comes back.
        ...
        if activity_id == "sputter_coat":
            ...
            if "current_ma" in params:
                parts.append(f"{params['current_ma']} mA")
            if "pressure_pa" in params:
                parts.append(f"{params['pressure_pa']} Pa")
        ...
        if activity_id == "argon_purge":
            ...
            if "cycles" in params:
                ...
                parts.append(
                    f"{cycles} cycle" + ("" if cycles == 1 else "s")
                )
            if params.get("chamber_recovery_s"):
                parts.append(f"recovery {params['chamber_recovery_s']} s")
            return ", ".join(parts)
