...
:class:`aquilos2_cu.pre_start_checks.checks.ZLinkedToFreeWorkingDistanceCheck`.
...
:class:`aquilos2_cu.pre_start_checks.checks.StagePositionWithinSafeRangeCheck`.
For a no-code toggle, the ``DEV_FORCE_UNLINKED`` and
``DEV_FORCE_STAGE_OUT_OF_RANGE`` flags in :mod:`aquilos2_cu.defaults`
...
class StageOps:
    ...
    def is_linked(self) -> bool:
        ...
        :func:`aquilos2_cu.pre_start_checks.snapshot.gather_snapshot`.
        """
    ...
    def lock_axis(self, axis: str) -> None:
        """Lock a stage axis via ``stage.safety_settings.lock_axis``.

        Used by Sputter Coat to immobilise the specimen (axes ``x``,
        ``y``, ``r``, ``t``) during magnetron plasma ignition, so the
        specimen can't drift in the high-energy environment.
        """
        logger.info("Stage: lock axis %r", axis)
        self._sdb.specimen.stage.safety_settings.lock_axis(axis)

    def unlock_axis(self, axis: str) -> None:
        """Unlock a stage axis previously locked via :meth:`lock_axis`."""
        logger.info("Stage: unlock axis %r", axis)
        self._sdb.specimen.stage.safety_settings.unlock_axis(axis)
...
class SimulatedStageOps:
    ...
    def __init__(self) -> None:
        ...
        # Axes currently locked via lock_axis() — lets the Sputter
        # Coat activity's lock/unlock (and its finally-unlock) be
        # exercised offline.
        self._locked_axes: set[str] = set()
    ...
    def lock_axis(self, axis: str) -> None:
        """Sim-only state: record ``axis`` as locked.

        Mirrors :meth:`StageOps.lock_axis` so Sputter Coat's axis
        lock/unlock (and its finally-unlock cleanup) can be exercised
        offline without hardware.
        """
        self._locked_axes.add(axis)
        logger.info(
            "Simulated stage: lock axis %r (locked=%r)", axis, self._locked_axes
        )

    def unlock_axis(self, axis: str) -> None:
        """Sim-only state: clear ``axis`` from the locked set."""
        self._locked_axes.discard(axis)
        logger.info(
            "Simulated stage: unlock axis %r (locked=%r)",
            axis, self._locked_axes,
        )
