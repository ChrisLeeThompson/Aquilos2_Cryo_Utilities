...
for the next pending activity. Exposes three activities in a fixed
order: Argon Purge, GIS Purge, then Home Stage. Mid-run switch toggles
take effect at the next fetch; the page has no mechanism for adding
activities.
...
from ..activities.argon_purge import ArgonPurgeService
...
_K_ARGON_PURGE_CYCLES = _PREFIX + "argonPurgeCycles"
_K_ARGON_PURGE_CHAMBER_RECOVERY_S = _PREFIX + "argonPurgeChamberRecoveryS"
...
class RTWorkflow(WorkflowRunner):
    ...
    argonPurgeEnabledChanged = Signal()
    argonPurgeCyclesChanged = Signal()
    argonPurgeChamberRecoveryChanged = Signal()
    ...
    # Live chamber-pressure telemetry for the Argon Purge chart:
    # (elapsed_seconds, pressure_pa), emitted ~once per 2 s poll while
    # argon purge runs. The ArgonPurgeService calls this signal's emit
    # from the worker thread; Qt delivers it to QML on the GUI thread via
    # a queued connection (same mechanism as progressUpdated/
    # statusUpdated). A QML chart in ArgonPurge.qml subscribes to it.
    pressureSampled = Signal(float, float)
    ...
    def __init__(
        ...
        self._argon_purge_enabled: bool = False
        ...
        self._argon_purge_cycles: int = self._read_int_clamped(
            _K_ARGON_PURGE_CYCLES,
            defaults.ARGON_PURGE_CYCLES_DEFAULT,
            defaults.ARGON_PURGE_CYCLES_MIN,
            defaults.ARGON_PURGE_CYCLES_MAX,
        )
        self._argon_purge_chamber_recovery: int = self._read_int_clamped(
            _K_ARGON_PURGE_CHAMBER_RECOVERY_S,
            defaults.ARGON_PURGE_CHAMBER_RECOVERY_DEFAULT_S,
        ...
        # stage-recorder lifecycle, keeping the two workflows symmetric.
        self._settings_snapshot: Optional[WorkflowSettingsSnapshot] = None

        # Stage-position recorder state — RT shares the
        # move-stage-to-original behavior with CPWorkflow: construct in
    ...
    def _any_activity_enabled(self) -> bool:
        return (
            self._argon_purge_enabled
            or self._gis_purge_enabled
            or self._home_stage_enabled
        )
    ...
    def _gather_pre_start_checks(self) -> List[PreStartCheck]:
        ...
        Each enabled activity class contributes its own checks (GIS
        Purge declares none). The orchestrator deduplicates by type,
        so multiple activities requesting the same check produce one
        evaluation.
        ...
        if self._argon_purge_enabled:
            checks.extend(ArgonPurgeService.pre_start_checks())
    ...
    def _before_run(self, on_status: StatusCallback) -> None:
        ...
        The only capture is the stage position, set up in
        :meth:`_on_commit_to_run` when move-stage-to-original is
        enabled. Best-effort: a capture failure is logged and the
        snapshot left None (restore skipped) without aborting the run.
        """
    ...
    def _after_run(self, on_status: StatusCallback, completed: bool) -> None:
        ...
        The only restore is the stage position, and only on a fully
        successful run (``completed`` is True). After a stop or
        exception the stage is left where it is — commanding further
        motion after an abnormal exit is unsafe. References (stage
        recorder/snapshot and the settings snapshot) are cleared
        unconditionally so nothing leaks into a
        subsequent run.
        """
        if completed:
            if (self._stage_recorder is not None
    ...
    def _next_pending_activity(
        ...
        RT exposes three single-instance activities in fixed order —
        Argon Purge (key ``"argon_purge/"``), GIS Purge
        (``"gis_purge/"``), then Home Stage (``"home_stage/"``). An
        activity toggled off after Start is simply not returned at the
        next fetch.
        ...
        """
        argon_purge_key = f"{ArgonPurgeService.activity_id}/"
        gis_purge_key = f"{GISPurgeService.activity_id}/"
        home_stage_key = f"{HomeStageService.activity_id}/"

        # Argon Purge runs first, matching the visual order on
        # RoomTempPrepPage.
        if (self._argon_purge_enabled
                and argon_purge_key not in executed_keys):
            if not self._mid_run_pre_start_check_passes(
                ArgonPurgeService, self._microscope,
            ):
                return self._abort_fetch()
            return self._build_argon_purge()
    ...
    def _build_argon_purge(self) -> ActivityService:
        """Construct an ArgonPurgeService from current parameter values.

        Wires the activity's per-poll pressure samples to
        :attr:`pressureSampled` so the QML chart can plot the chamber
        pressure live. The bound ``.emit`` is called from the worker
        thread; Qt queues it to the GUI thread (same pattern as the
        runner forwarding ``progressUpdated.emit`` into ``run()``).
        """
        return ArgonPurgeService(
            vacuum_ops=self._microscope.vacuum,
            cycles=self._argon_purge_cycles,
            chamber_recovery_s=self._argon_purge_chamber_recovery,
            on_sample=self.pressureSampled.emit,
        )
    ...
    # --- Argon Purge — enabled flag ---------------------------------------

    @Property(bool, notify=argonPurgeEnabledChanged)
    def argonPurgeEnabled(self) -> bool:
        return self._argon_purge_enabled

    @argonPurgeEnabled.setter
    def argonPurgeEnabled(self, value: bool) -> None:
        value = bool(value)
        if value == self._argon_purge_enabled:
            return
        self._argon_purge_enabled = value
        self.argonPurgeEnabledChanged.emit()
        self.canStartChanged.emit()

    # --- Argon Purge — cycles ---------------------------------------------

    @Property(int, notify=argonPurgeCyclesChanged)
    def argonPurgeCycles(self) -> int:
        return self._argon_purge_cycles

    @argonPurgeCycles.setter
    def argonPurgeCycles(self, value: int) -> None:
        value = self._clamp(
            int(value),
            defaults.ARGON_PURGE_CYCLES_MIN,
            defaults.ARGON_PURGE_CYCLES_MAX,
        )
        if value == self._argon_purge_cycles:
            return
        self._argon_purge_cycles = value
        self._qs.setValue(_K_ARGON_PURGE_CYCLES, value)
        self.argonPurgeCyclesChanged.emit()

    # --- Argon Purge — chamber recovery -----------------------------------

    @Property(int, notify=argonPurgeChamberRecoveryChanged)
    def argonPurgeChamberRecovery(self) -> int:
        return self._argon_purge_chamber_recovery

    @argonPurgeChamberRecovery.setter
    def argonPurgeChamberRecovery(self, value: int) -> None:
        value = self._clamp(
            int(value),
            defaults.CHAMBER_RECOVERY_MIN_S,
            defaults.CHAMBER_RECOVERY_MAX_S,
        )
        if value == self._argon_purge_chamber_recovery:
            return
        self._argon_purge_chamber_recovery = value
        self._qs.setValue(_K_ARGON_PURGE_CHAMBER_RECOVERY_S, value)
        self.argonPurgeChamberRecoveryChanged.emit()
    ...
    def reset_parameters(self) -> None:
        ...
        self.argonPurgeCycles = defaults.ARGON_PURGE_CYCLES_DEFAULT
        self.argonPurgeChamberRecovery = (
            defaults.ARGON_PURGE_CHAMBER_RECOVERY_DEFAULT_S
        )
