...
Currently exposes three activities: Argon Purge, GIS Purge, and Home
Stage.

Iterative-fetch architecture
----------------------------
RT walks a static ordered list (Argon Purge, then GIS Purge, then Home
Stage) in :meth:`_next_pending_activity`. Mid-run switch toggles fold
in for free — a disabled activity isn't pending and isn't returned. The
page has no mechanism for adding activities mid-run (unlike the Cryo
page), so the static order is fixed.
...
class RTWorkflow(WorkflowRunner):
    ...
    def _on_commit_to_run(self) -> None:
        ...
        See
        :mod:`aquilos2_cu.workflows.settings_snapshot.WorkflowSettingsSnapshot`
    ...
    def _next_pending_activity(
        ...
        # Argon Purge runs first (v2.5 ordering), matching the visual
        # order on RoomTempPrepPage.
        if (self._argon_purge_enabled
