...
def template_dict_from_record(
    ...
    2. For GIS records, ``position_id`` is replaced by
       ``position_name`` resolved through
...
def record_from_template_dict(
    ...
    ``position_name_remap`` (GIS only) maps a template position name to
    a local id for the just-imported positions — see
    :func:`parse_template_payload`.
    ...
    if activity_type == ACTIVITY_TYPE_SPUTTER_COAT:
        return _sputter_record_from_template_dict(d, issues)
...
def _sputter_record_from_template_dict(
    d: Dict[str, Any], issues: List[str],
) -> SputterCoatRecord:
    """Build a fresh (magnetron) :class:`SputterCoatRecord` from a template."""
    current_ma = _coerce_int(
        d, "current_ma",
        defaults.SPUTTER_CURRENT_DEFAULT_MA, issues,
    )
    current_ma = _clamp(
        current_ma,
        defaults.SPUTTER_CURRENT_MIN_MA,
        defaults.SPUTTER_CURRENT_MAX_MA,
        issues, "Sputter Coat current_ma",
    )
    pressure_pa = _coerce_int(
        d, "pressure_pa",
        defaults.SPUTTER_PRESSURE_DEFAULT_PA, issues,
    )
    pressure_pa = _clamp(
        pressure_pa,
        defaults.SPUTTER_PRESSURE_MIN_PA,
        defaults.SPUTTER_PRESSURE_MAX_PA,
        issues, "Sputter Coat pressure_pa",
    )
...
def _coerce_float(
    ...
    # non-finite value must never reach a hardware write. Fall back to
    # the default with a recorded issue.
