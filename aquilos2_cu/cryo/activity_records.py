...
class SputterCoatRecord:
    """Parameters for a single (magnetron) Sputter Coat activity instance.

    The Aquilos 2 magnetron coater is parameterized by current (mA),
    target chamber pressure (Pa), sputter duration (s), and a post-run
    chamber recovery (s). It does not use the ion beam, an ion species,
    a grid selection, or a high voltage.
    """

    instance_id: str = field(default_factory=_new_instance_id)
    # Magnetron sputter current in milliamperes (whole numbers in the
    # UI; the activity converts to amperes before writing to hardware).
    current_ma: int = defaults.SPUTTER_CURRENT_DEFAULT_MA
    # Target chamber pressure in pascals (argon).
    pressure_pa: int = defaults.SPUTTER_PRESSURE_DEFAULT_PA
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "SputterCoatRecord":
        ...
        # persisted data without a field doesn't crash on load. Records
        # written by the Hydra-era schema (grid_index / ion_species_index
        # / ion_current_a) simply fall through to the new defaults here;
        # those keys are ignored.
        return cls(
            instance_id=str(d.get("instance_id") or _new_instance_id()),
            current_ma=int(d.get(
                "current_ma", defaults.SPUTTER_CURRENT_DEFAULT_MA
            )),
            pressure_pa=int(d.get(
                "pressure_pa", defaults.SPUTTER_PRESSURE_DEFAULT_PA
            )),
