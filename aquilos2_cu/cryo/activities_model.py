...
roles, so QML delegates can bind directly to ``model.sputterCurrentMa``,
``model.sputterPressurePa``, ``model.gisDuration``, etc.
...
class CryoActivitiesModel(QAbstractListModel):
    ...
    # Sputter Coat (magnetron) parameters.
    SputterCurrentMaRole = Qt.UserRole + 10
    SputterPressurePaRole = Qt.UserRole + 11
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole) -> Any:
        ...
        if isinstance(rec, SputterCoatRecord):
            if role == self.SputterCurrentMaRole:
                return rec.current_ma
            if role == self.SputterPressurePaRole:
                return rec.pressure_pa
    ...
    def roleNames(self) -> Dict[int, QByteArray]:
        ...
        return {
            ...
            self.SputterCurrentMaRole: QByteArray(b"sputterCurrentMa"),
            self.SputterPressurePaRole: QByteArray(b"sputterPressurePa"),
