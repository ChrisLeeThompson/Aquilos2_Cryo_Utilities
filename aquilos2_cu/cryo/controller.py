...
class CryoActivitiesController(QObject):
    ...
    def sputterCurrentMin(self) -> int:
        return defaults.SPUTTER_CURRENT_MIN_MA

    @Property(int, constant=True)
    def sputterCurrentMax(self) -> int:
        return defaults.SPUTTER_CURRENT_MAX_MA

    @Property(int, constant=True)
    def sputterPressureMin(self) -> int:
        return defaults.SPUTTER_PRESSURE_MIN_PA

    @Property(int, constant=True)
    def sputterPressureMax(self) -> int:
        return defaults.SPUTTER_PRESSURE_MAX_PA

    @Property(int, constant=True)
    def sputterPressureStep(self) -> int:
        return defaults.SPUTTER_PRESSURE_STEP_PA
    ...
    def templatesDirUrl(self) -> QUrl:
        ...
        (``aquilos2_cu/cryo/controller.py`` -> ``aquilos2_cu/`` ->
    ...
    def reset_parameters(self) -> None:
        ...
        For GIS Deposition activities, ``position_id`` is *preserved*
        (not reset). The position is closer to a structural choice
        (pointing at a saved entity outside the activity) than to a
        per-activity parameter, and preserving it matches user
        intuition for "reset parameters" — the activity stays
        targeting the same place, just with default duration and
        recovery values.
    ...
    def set_sputter_current_ma(self, instance_id: str, value: int) -> bool:
        """Set the magnetron sputter current in milliamperes (clamped)."""
        clamped = _clamp(
            int(value),
            defaults.SPUTTER_CURRENT_MIN_MA,
            defaults.SPUTTER_CURRENT_MAX_MA,
        )
        return self._update_record(
            instance_id, SputterCoatRecord, current_ma=clamped
        )

    @Slot(str, int, result=bool)
    def set_sputter_pressure_pa(self, instance_id: str, value: int) -> bool:
        """Set the sputter target chamber pressure in pascals (clamped)."""
        clamped = _clamp(
            int(value),
            defaults.SPUTTER_PRESSURE_MIN_PA,
            defaults.SPUTTER_PRESSURE_MAX_PA,
        )
        return self._update_record(
            instance_id, SputterCoatRecord, pressure_pa=clamped
        )
    ...
    def _referenced_positions(
        ...
        """The saved positions the GIS Deposition activities point at.

        De-duplicated and order-preserving. A GIS activity with no
        position selected contributes nothing; a stale ``position_id``
        that no longer resolves is skipped (it can't be bundled, and it
        already surfaces at workflow start).
        """
        if self._positions is None:
            return []
        seen = set()
        ...
        for rec in records:
            if not isinstance(rec, GISDepositionRecord):
                continue
            pid = rec.position_id
