...
// include Argon Purge, GIS Purge, and Home Stage (run in that order).
//
...
Item {
    ...
    Connections {
        ...
        function onWorkflowStarted() {
            ...
            if (argonPurgeActivityContainer.switchChecked) {
                argonPurgeActivityContainer.activityState = "idle"
                argonPurgeActivityContainer.statusMessage = ""
            }
        ...
        function onActivityStatusChanged(activity_id, instance_id, status) {
            ...
            let container = null
            if (activity_id === "argon_purge") {
                container = argonPurgeActivityContainer
            } else if (activity_id === "gis_purge") {
            ...
            if (status === "complete") {
                ...
                if (activity_id === "argon_purge") {
                    appController.rtWorkflow.argonPurgeEnabled = false
                } else if (activity_id === "gis_purge") {
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            ActivityContainer {
                ...
                id: argonPurgeActivityContainer
                title: "Argon Purge"
                titleToolTip: Strings.argonPurgeActivityTooltip
                Layout.fillWidth: true
                Layout.maximumWidth: AppConfig.activityContainerMaxWidthRoomTemp
                Layout.alignment: Qt.AlignHCenter
                expanded: true
                parameterSummary: argonPurgeActivity.parameterSummary
                stackHeader: true

                // Same switch enable rule as the other RT activities —
                // unlocked between runs, and during a run only while the
                // activity is still idle (hasn't started). See the GIS
                // Purge container below for the full rationale.
                ...
                onToggled: function(enabled) {
                    if (appController.rtWorkflow) {
                        appController.rtWorkflow.argonPurgeEnabled = enabled
                    }
                ...
                ArgonPurge {
                    ...
                    id: argonPurgeActivity
                    Layout.fillWidth: true

                    // Light/dim the chart with the visible enable toggle.
                    activityEnabled: argonPurgeActivityContainer.switchChecked
