...
<!-- Full documentation: https://<site>/scripts/aquilos2_cryo_utilities/ (enable this link when the site is live) -->

A PySide6/QML desktop utility for the Thermo Scientific Aquilos 2 cryo FIB-SEM. It supports common cryo workflows on the microscope with automated preparation activities, stage movement controls, and angle calculators, and it runs through the Thermo Scientific AutoScript SDK.

## Features

- **RT Prep** activities for a room-temperature stage: argon purge, GIS purge, and home stage.
- **Cryo Prep** activities for a cold stage: stage positions, sputter coat, GIS deposition, and home stage, with an activity list to run them in sequence.
...
- **Milling Angle Calc** with interactive graphics showing stage tilt relative to the SEM, FIB, GIS, and milling positions.
...
1. Download the latest release ZIP from the [Releases page](https://github.com/ChrisLeeThompson/Aquilos2_Cryo_Utilities/releases).
...
```
python aquilos2_cryo_utilities.py
```
...
```
python aquilos2_cryo_utilities.py --simulation
```

In simulation mode the UI is fully navigable but cannot control a microscope. The script also falls back to simulation mode on its own when it cannot connect to a microscope, and shows "Microscope not available" in the status bar. To force simulation mode persistently, set `DEV_FORCE_SIMULATION` to `True` in `aquilos2_cu/defaults.py`.
...
- Saved templates in `templates/` are never overwritten. The shipped default template lives inside the app (`aquilos2_cu/cryo/factory_templates/`) and is copied into `templates/` at startup only when a file with that name is missing. If you delete `templates/default.json`, it reappears on the next launch; to customize the default, edit it and save over it.
...
What is replaced: the application code, including the factory templates inside `aquilos2_cu/`.
